...
async def test_mac(dut):
    ...
    for i in range(0,1052):
        if i<1049:
            dut.start_a.value = A_dec_int[i]
            dut.start_b.value = B_dec_int[i]
            dut.start_c.value = C_dec_int[i]
        await RisingEdge(dut.CLK)
        if i>2:
            dut._log.info(f'output {dut.get_result.value}')
            assert int(MAC_bin_int[i-3]) == int(str(dut.get_result.value)), f'MAC Output Mismatch, Expected = {MAC_bin_int[i-3]} DUT = {int(str(dut.get_result.value))}'
    ...
    for i in range(0,1003):
        if i<1000:
            dut.start_a.value = A_bin_fp[i]
            dut.start_b.value = B_bin_fp[i]
            dut.start_c.value = C_bin_fp[i]
        await RisingEdge(dut.CLK)
        if i>2:
            dut._log.info(f'output {dut.get_result.value}')
            assert str(MAC_bin_fp[i-3])[0:30] == str(dut.get_result.value)[0:30], f'MAC Output Mismatch, Expected = {str(MAC_bin_fp[i-3])} DUT = {str(dut.get_result.value)}' 
