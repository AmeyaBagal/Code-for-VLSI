...
async def test_mac(dut):
    ...
    for i in range(0,1057):
        ...
        if i>7:
            ...
            assert int(MAC_bin_int[i-8]) == int(str(dut.get_result.value)), f'MAC Output Mismatch, Expected = {MAC_bin_int[i-8]} DUT = {int(str(dut.get_result.value))}'
